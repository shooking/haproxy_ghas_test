                                          ...
                                          char **err)
{
	...
	if (global_ssl.ocsp_update.delay_min > value) {
		memprintf(err, "'%s' can not be lower than tune.ssl.ocsp.mindelay.", args[0]);
		return -1;
	}
                                          ...
                                          char **err)
{
	...
	if (value > global_ssl.ocsp_update.delay_max) {
		memprintf(err, "'%s' can not be higher than tune.ssl.ocsp.maxdelay.", args[0]);
		return -1;
	}
...
static struct cfg_kw_list cfg_kws = {ILH, {
	...
	{ CFG_GLOBAL, "tune.ssl.ocsp.maxdelay", ssl_parse_global_ocsp_maxdelay },
	{ CFG_GLOBAL, "tune.ssl.ocsp.mindelay", ssl_parse_global_ocsp_mindelay },
#endif
	{ 0, NULL, NULL },
}};
