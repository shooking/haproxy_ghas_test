...
static int dump_ocsp_update_info(struct certificate_ocsp *ocsp, struct buffer *out)
{
	...
	get_gmtime(ocsp->next_update.key, &tm);
	ret = gmt2str_log(b_orig(out)+b_data(out), &tm, b_size(out)-b_data(out));
	...
	if (ocsp->last_update) {
		get_gmtime(ocsp->last_update, &tm);
		ret = gmt2str_log(b_orig(out)+b_data(out), &tm, b_size(out)-b_data(out));
...
static int cli_io_handler_show_ocsp_updates(struct appctx *appctx)
{
	...
	if (!ctx->cur_ocsp) {
		node = eb64_first(&ocsp_update_tree);
		chunk_appendf(trash, "OCSP Certid | Next Update | Last Update | Successes | Failures | Last Update Status | Last Update Status (str)\n");
